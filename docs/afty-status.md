...
## SESSÃO DE 2026-09-05 (parte 5): A FICHA SEM NOME SAÍA DO APP E NÃO VOLTAVA

O autor publicou a atualização e as fichas novas passaram a falhar na IMPORTAÇÃO, com
`Criatura inválida:` seguido do JSON inteiro.

### O diagnóstico, medido do começo ao fim

A ficha estava **saudável**. Rodei o JSON dele pelo caminho completo:

| passo | resultado |
|---|---|
| importar como veio | **REJEITADA** |
| importar trocando só o `name` | passou |
| derivar | PV 33, PE 15, Defesa 15, CD 17, 20 perícias, zero problema |

E os quatro números batem com o snapshot `stats` gravado na própria ficha. Um campo só era o
culpado: **`"name": ""`**.

O `parseImportText`, em `src/components/io-utils.js`, reprova criatura com nome vazio. E o criador do
Afty nunca exigiu nome: o `createBlankAfty` nasce com `name: ""` e o `handleSave` gravava o rascunho
como estava. Dava para montar a ficha toda, salvar, exportar, e só descobrir o problema do outro
lado.

⚠ **E O ESTRAGO É MAIOR QUE UMA FICHA.** O importador **LANÇA em vez de pular**: uma ficha sem nome
no meio de um arquivo derruba a importação inteira, levando junto todas as outras que vieram no mesmo
pacote. Está medido em assert: três fichas, uma sem nome, e entram zero.

### O conserto, e onde ele mora

`nomeParaGravar(nome)`, em **`afty-schema.js`**, junto do `createBlankAfty` que cria o campo vazio.
O `handleSave` passa a usá-la.

⚠ **A REGRA NÃO FICOU DENTRO DO `handleSave`**, e a razão é o assert: uma regra escrita dentro de um
componente JSX não pode ser medida, e reescrevê-la no teste mediria a cópia. Regra escrita em dois
lugares é a regra que diverge no primeiro conserto.

⚠ **"Sem nome" não é rótulo novo.** É o que o Preview já escreve no lugar do nome, e o que a lista de
Invocações usa. Inventar outro criaria um segundo vocabulário para a mesma ideia.

⚠ **Espaço em branco também é aparado**, e este é o caso traiçoeiro: `"   "` PASSA no importador,
porque para ele basta ser string não vazia. A ficha entraria com nome invisível e sumiria na lista do
Dashboard. Aparar resolve os dois de uma vez.

### O assert importa o `io-utils.js` da 2.5.2, de propósito

Nasceu **`t-nome-ficha.mjs`** (16). Ele não pergunta "o nome ficou certo", pergunta **"a ficha
atravessa a importação"**, chamando o `parseImportText` de verdade.

⚠ Reescrever ali a regra do importador mediria a minha cópia dela, e no dia em que ele mudasse de
ideia o arquivo continuaria verde mentindo. **Importar de `src/components/` é permitido: o que a
regra da casa proíbe é EDITAR.**

### O que NÃO foi consertado, e por quê

As fichas antigas, já exportadas com `name: ""`, continuam sendo recusadas, e um pacote inteiro
continua morrendo por causa de uma entrada. Os dois consertos moram no `parseImportText`, que é da
2.5.2 e somente-leitura, e **o import não tem nenhum ponto de entrada do lado do Afty**: o `throw`
acontece antes de qualquer código que o Afty controle. Virou pergunta em `a-fazer.md`, com três
saídas.

### Verificação

`eslint` limpo, `vite build` ok, **56 arquivos e 2903 asserts** (eram 55 e 2887).
`src/components/` intocado.

**Navegador**: criei uma ficha em `/Player` e em `/Afty` e salvei **sem digitar nome**. Nos dois, o
que foi para o `localStorage` é `name: "Sem nome"`, e a ficha aparece na lista com esse nome. Zero
erro de console.
...
## SESSÃO DE 2026-09-22: MALDIÇÃO - ERA DE OURO (5 TIPOS DE ESPÍRITO AMALDIÇOADO)

Pedido do autor, com o PDF do suplemento colado (extraído por script próprio nesta sessão, sem
poppler disponível na máquina: leitura direta dos objetos do PDF via `zlib.inflateSync` e dos
CMaps ToUnicode de cada fonte). Antes de escrever qualquer coisa, a sessão descobriu que boa parte
...
## SESSÃO DE 2026-09-23: REVISÃO DO ESPECIALISTA EM COMBATE CONTRA O LIVRO (ETAPA 1)

Pedido do autor: revisar a classe inteira (no código, `combatente` e prefixo `cmb_`) contra o Livro F&M 2.5.2, sem recriá-la, corrigindo só o que estivesse comprovadamente errado e automatizando o que é objetivo. Decisões dele na abertura: as correções valem para criatura e jogador; o "Pré-Requisito: Nível N" das opções usa o nível de escalonamento (*"O padrão que usamos"*); o Teste de Resistência Mestre volta só no jogador, em etapa própria; e os Pontos de Preparo viram contador.

**Conferido e correto (sem edição):** PV 12 / 1d10 / 6, PE 4 por nível, armas e escudos, TR Fortitude ou Reflexos, as 6 perícias do pacote, multiclasse FOR ou DES 16. As 64 habilidades por nível estão no catálogo, nos níveis e com os pré-requisitos do livro. Progressão de Estilos (1, 6, 12 e Acervo Amplo), de Posturas (ao obter, 8, 16 e Aprender Postura no 4 e no 10), degraus dos Estilos, Implemento Marcial, Preparo, Execução Silenciosa, Golpe Descendente, Críticos, Aptidões de Combate, Revigorar, tetos do Pistoleiro, do Ataque Concentrado e da Precisão.

**Corrigido:**
- **Bônus do Combatente vazando para Feitiço.** Autossuficiente, Arsenal Cíclico, Estilo Duplo, Estilo do Duelista e Precisão Definitiva eram efeitos sem alvo, e o resolve do Feitiço lê o valor sem alvo junto do dele (`valorCanalEscopos`). No nível 20 um Feitiço de Nível 1 saía com 5d8 em vez de 4d8, com "Autossuficiente +1d8" no hover, e o Duelista somava +6 no dano e +3 na jogada Amaldiçoada. Agora os cinco miram `arma` e `basico`. O Básico fica pela decisão do autor de 2026-09-16, presa em `t-niveis-dano.mjs`, que pegou a primeira versão desta correção (só `arma`) na hora. O acerto do Duelista e da Precisão passou de `bonusAcerto` para `acertoArma`, o canal que a linha soma por cima da jogada do tipo de ataque. Arma e Básico ficaram com os mesmos números de antes.
- **Espírito Incansável no jogador.** Os PV temporários "se tornam o seu bônus de ataque", e a expressão remontava esse bônus com `piso(nd / 1.5)`, a régua da criatura. Variável nova `escala_ataque` no DSL (`piso(nd / 1.5)` na criatura e `piso(nd / 2)` no jogador, a mesma conta do `resolveTestes`). No jogador os PV temporários agora são o acerto da arma: 15, 18 e 24 nos níveis 8, 12 e 20, contra 16, 20 e 27 antes. A criatura não mudou.
- **"Pré-Requisito: Nível N" das opções, que ninguém lia.** `nivelMin` existia na Postura da Devastação (6), da Tempestade (10) e do Céu (12), no Feitiço Rápido (Conjurador, 6) e no Apoio Estratégico (Suporte, 6). `resolveEscolhasHabilidade` agora devolve `bloqueadas` e `abaixoDoNivel` pelo nível de escalonamento da Especialização dona, e o card da escolha aninhada desabilita a opção trancada ("Requer Nível N"). A já gravada abaixo do nível fica na ficha, marcada em âmbar, igual aos `inacessiveis`. Conjurador e Suporte ganham a mesma trava.

**Melhoria:** o Grupo Favorito ganhou a escolha de grupo ("Escolha um grupo de armas"), com o pool do Armas Escolhidas e prefixo próprio (`cmb_favorito_`), sem efeito de Motor (o efeito de crítico do grupo não tem canal).

**Automação:** Pontos de Preparo como recurso de sessão. Campo `preparoAtual` (`null` = cheio) e os verbos `preparoDe`, `alteraPreparo` e `definePreparo` em `ficha-sessao.js`. Nunca passa do máximo nem desce de zero, é aparado quando o máximo cai e volta cheio no Descansar. Barra própria na Ficha Final e no painel de Encontros, no lugar do ladrilho `[data-afty-stat="preparo"]`. É a única quebra da lista de ganchos estáveis, registrada no guia da Ficha.

**Ficou para o autor** (entrada nova no `a-fazer.md`): o Preparo na multiclasse lido pelo escalonamento, a Postura da Lua sem o atributo no dano, Guarda Estudada e Mente Oculta com SAB negativa, o +2 da Extensão do Corpo só no corpo a corpo, os "a cada 4 níveis", o Golpes Potentes sem conferir treino, o Espírito Incansável sempre por Força, o atributo-chave da classe, os contadores de uso e o temporário da Postura do Céu.

**Asserts:** `t-combatente-revisao.mjs` (novo, 67), que mede a progressão nos níveis 1, 4, 6, 8, 12, 16 e 20, o vazamento, os dois sistemas do Espírito Incansável, a trava com multiclasse, o Grupo Favorito e o Preparo na sessão. Contraprova contra o commit anterior: 22 falham lá. A suíte completa ficou igual à linha de base (112 → 113 arquivos, o único vermelho segue o conhecido `t-invocacoes-motor.mjs`, com 1 falha e 84 ok antes e depois).

**Verificado ao vivo** (`/Player`, dev limpo): no Combatente 20, barra de Preparo 22 (20 + SAB 2), 3 gastos levam a 19, o `+` para em 22, 25 gastos param em 0, o Descansar volta a 22 e a sessão grava `null`, e o Feitiço de Nível 1 mostra 4d8. No criador do Combatente 11, a Postura do Céu fica desabilitada com "Requer Nível 12" e a da Tempestade liberada, e o Grupo Favorito lista os grupos. Console limpo. `npx eslint src/systems/afty` zerado e `npm run build` limpo.

### Etapa 2 (mesmo dia): o Teste de Resistência Mestre volta no jogador

Feita depois da Etapa 1 validada, e separada porque mexe nas seis classes. Livro, nas cinco: *"No nível 9, você recebe a habilidade Teste de Resistência Mestre: Você se torna treinado em um segundo teste de resistência e mestre no concedido pela sua especialização."* No Restringido: *"mestre nos dois Testes de Resistência conferidos por sua Especialização"*.

- **O bloqueio era de dado:** a ficha de jogador não sabia qual TR a classe tinha dado. Quem tem "Fortitude ou Reflexos" marcava à mão, e a marcação é livre em qualquer TR e faixa. O autor escolheu **registrar a escolha** e decidiu que o Mestre é **só da Classe inicial**, a única que concede TR.
- **Dado:** `caracteristicas.resistencias.mestre = { nivel: 9, segundo }` nas seis (`segundo: false` só no Restringido), com o validador aceitando o campo. Campos novos na ficha: `trDaClasse` (lista) e `trSegundo`.
- **Motor:** `resistenciasDaClasse` (`afty-especializacoes.js`) devolve as faixas por TR, as pendências e as opções. O `pacoteInicialDaFicha` passou a levar `trMestre` e `nivelClasse`. O `resolveTestes` deixou o conjunto `trDoPacote` (tudo treinado) e passou a ler um mapa de faixas, e o `profComEfeito` recebe a faixa em número. Divergência nova `trMestreDoJogador`, e a criatura não muda.
- **Tela** (`EscolhasDoTrDaClasse`, no card de Testes de Resistência do jogador): "TR da Classe" entre os da lista, e a partir do 9 "Segundo TR" entre os outros, pedido só depois do da Classe. O aviso âmbar aparece para a escolha que falta.
- **Asserts:** `t-tr-mestre-jogador.mjs` (novo, 22). `t-sistema.mjs` ganhou a divergência nas duas listas (449 → 453). `t-pericias-pacote.mjs` media o Restringido de nível 10 com os TR "treinado", e passou a esperar "mestre", com a nota do porquê. O que ele mede (os dois chegam sem marcar e concedidos) continua igual. Contraprova: o motor anterior nem monta o `trDaClasse`.
- **Verificado ao vivo** (`/Player`, Combatente 9): só "TR da Classe" aparece de início; escolhida a Fortitude, aparece "Segundo TR" sem ela; escolhida a Vontade, o aviso some. Na aba e na Ficha Final, depois de salvo: Fortitude Mestre +10 com Sucesso Crítico, Vontade +8, o resto +4. Console limpo numa aba nova. Os dois erros do Vite na aba antiga eram do servidor anterior, parado com a página aberta.
- **Ficou para o autor:** a marcação à mão de TR no jogador segue livre em qualquer faixa (entrada nova no `a-fazer.md`).

### Fechamento dos pontos de verificação do Especialista em Combate (mesmo dia)

Pedido do autor: fechar só os pontos que a revisão deixou abertos, cada um classificado como regra confirmada, decisão do autor, não automatizável, automação futura ou erro confirmado, e mexer em código só onde o livro sustenta uma diferença objetiva. Duas frases das regras de Multiclasse decidiram vários: *"Ao obter uma nova especialização, você não recebe novos treinamentos em perícias nem equipamentos"* e *"considera-se seu nível da Multiclasse + Metade do seu Nível em outras Especializações para efeitos de habilidades, mas não para efeitos de pré-requisito"*.

**Erros confirmados e corrigidos:**
- **Estilo Massivo** → duas linhas de efeito (`prop:duas_maos` e `prop:pesada`) → uma linha com o alvo `empunho:duas_maos|prop:pesada`. As dez armas Pesadas e de Duas Mãos (Espada Grande, Machado Grande...) recebiam o bônus duas vezes (+10 no nível 16, onde o texto dá +5), e a Versátil nas duas mãos não recebia nada. Motor: o alvo com "ou" nos leitores por escopo (entra uma vez só) e o escopo `empunho:duas_maos` em `escoposDaArma`.
- **Postura da Lua (jogador)** → o atributo continuava no dano → canal novo `semAtributoDano`, lido só na linha de dano do jogador, respeitando o Invencível sob o Sol como o −4 já respeitava. O hover mostra "Força (Postura da Lua)" com 0. Na criatura o canal é ignorado, porque lá o atributo decide também quantos dados a linha rola, e a leitura ficou para o autor.
- **Precisão Definitiva (multiclasse)** → o teto de PE lia o nível real → lê o escalonamento, no derive e no `max` da bancada (`nivelDeEscalonamento`, sem import novo no `afty-combate.js`). Combatente 4 / Lutador 8 ia de teto 2 para 3. O Revigorar, com a mesma frase, já lia o escalonamento.

**Regra confirmada, sem mudança:** Guarda Estudada e Mente Oculta aplicam SAB negativa (o livro escreve "com o mínimo de 1" quando quer piso, e aplica negativo na carga), Pontos de Preparo pelo escalonamento na multiclasse (é a regra de Multiclasse) e Mestre Pistoleiro como melhora (a tabela de Feitiço usa "aumentar a margem de crítico" como melhora). Só o comentário do Mestre Pistoleiro mudou.

**Decisão do autor, automação futura e mesa:** reorganizados na entrada do Combatente em `docs/a-fazer.md`, com a classificação de cada um. Uma correção de registro: a entrada anterior dava o Mestre Pistoleiro como "já anotado", e não estava. Quatro entradas novas saíram dos achados, todas fora do Combatente e sem mexer em código: o treino de arma na multiclasse (o livro não dá treino novo), a CD de Especialização separada da CD Amaldiçoada, a Brutalidade do Lutador ainda pelo nível real, e quantas escolhas aninhadas a multiclasse dá (o livro se contradiz entre a frase do "fortalecimento" e o exemplo do Treinamento em Controle).

**Asserts:** `t-combatente-revisao.mjs` de 67 para 79. Contraprova contra o motor anterior: os 7 casos que medem os consertos falham lá, e os que medem "não muda" (Feitiço, criatura, classe única) passam dos dois lados. Canais: 98, 45 com alvo.

---

## SESSÃO DE 2026-09-23 E 24: AS AUTOMAÇÕES DO ESPECIALISTA EM COMBATE

Pedido do autor, depois do fechamento dos pontos de verificação: *"inicie a modificação de tudo disponível, e que pode ser automatizado pfvr, e faça perguntas sobre todas as duvidas que tiver enquanto inicia os trabalhos"*. Foi feito item a item, com uma pergunta ao autor antes de cada decisão de regra. Nada commitado.

**As respostas do autor** (2026-09-23, salvo onde marcado):
- Descanso (a D3 da Ficha Final): *"Manter um botão só"*. O Descansar devolve tudo, e a recarga do livro fica gravada como dado.
- Contador de usos: *"Na linha da habilidade"*, e *"Só o Combatente agora"*.
- Golpe Especial: *"Montador completo"*. Os PE do Autossuficiente *"Abatem o custo do Golpe"*, e as marcas *"Ficam marcadas até desmarcar"*. O Longo vale *"Nos dois alcances"*.
- Postura do Céu: a casca *"Topa em 2"*.
- Treino de equipamento na multiclasse: *"Só a classe inicial treina"* no jogador, e na criatura *"O catálogo das classes"*.
- Brutalidade: *"Sim, pelo escalonamento"*. Espírito Incansável: *"O maior entre FOR e DES"*.
- Sem mudança de código: Postura da Lua na criatura *"Deixar como está"*, o +2 da Extensão do Corpo *"Manter no Corpo a Corpo"*, o "a cada 4 níveis" *"Nos níveis 4, 8, 12, 16 e 20"*, e a CD de Especialização *"Não fazer agora"*.
- 2026-09-24: as vagas por nível das Bases na multiclasse ficam pelo *"Nível real (como hoje)"* (a linha 1378 do livro vence o exemplo da 1382), e o TR marcado à mão no jogador passa a *"Avisar em âmbar"*.

**Feito, estado antes → depois:**
- **Canal `alcanceArma`** (novo, fonte de dano) → a Extensão do Corpo e a Sincronia Perfeita só tinham texto, e agora somam 1,5 cada nas armas corpo a corpo. O canal soma nos dois alcances da arma de distância e entra antes do dobro da Postura do Céu. O texto do alcance ganhou vírgula ("4,5m").
- **Manejo Único** → sem a segunda propriedade → `concedeEscolha` no card do Manejo Especial (duas vagas no nível 12), como o Acervo Amplo faz com os Estilos.
- **Brutalidade (Lutador)** → degrau pelo nível real → pelo escalonamento (`nivelDeEscalonamento`), no derive e no `max` da bancada. Lutador 8 com Combatente 8 vai de 1 para 2 degraus.
- **Treino de equipamento no jogador** → juntava todas as classes → só a Classe inicial (divergência nova `treinoDaClasseInicial`, livro: *"Ao obter uma nova especialização, você não recebe novos treinamentos em perícias nem equipamentos"*). Escopo novo `treinada` em `escoposDaArma`, e o Golpes Potentes passou a mirar só a arma treinada.
- **Espírito Incansável** → lia a Força → o maior entre Força e Destreza.
- **Postura do Céu** → +2 no MÁXIMO de Preparo → casca de 2 temporários por rodada (canal novo `preparoTemporario`, sem alvo). A sessão guarda `preparoTemp`, topa em 2 na virada de rodada e no começo do combate, gasta a casca antes do corrente, apara ao sair da Postura e zera no Descansar. A barra mostra a casca como a de PV e PE.
- **Contador de usos** → só o Revigorar tinha número (o máximo, na linha de cura) → campo `usos: { expr, recarga }` no catálogo de Habilidades, com validador, `usosHabilidades` no derive, "Usos restantes/máximo" com menos e mais na linha da Habilidade e no Rápido, e os gastos em `usos["hab:<id>"]` na sessão. As seis do Combatente: Assumir Postura, Indomável, Revigorar, Marcar Inimigo, Surto de Ação e Potência Antes de Cair.
- **Montador do Golpe Especial** → 4 interruptores soltos na aba Buffs e custo em lugar nenhum → `PainelDoGolpeEspecial.jsx` na aba Ações (Ficha e Encontro) com as onze propriedades, custo total com mínimo de 1 PE, Autossuficiente (3, ou 6 uma vez por cena), Preciso que custa 2 depois do primeiro da rodada, Sacrifício cobrando 15 de dano pelo `aplicaDano`, e o Pagar pelo `gastaPe`. O Longo virou estado de bancada (`golpeLongo`, com o `alcanceArma`). ⚠ As seis marcas sem número moram em `sessao.golpeEspecial`, fora da bancada: `golpeImpactante` já é o id do Golpe Impactante do Restringido. Catálogo e conta em `afty-golpe-especial.js` (folha).
- **TR marcado à mão no jogador** → livre e calado → `semFonte` no `resolveTestes` quando a marcação passa do que a Classe e o Motor dão, e a linha do criador mostra "Sem Fonte" em âmbar. Nada é tirado, e marcar o que uma fonte já dá não acusa.

**Asserts:** `t-combatente-automacoes.mjs` (novo, 73) cobre o alcance, o Manejo Único, a Brutalidade, o treino e o Golpes Potentes, a casca do Céu, o contador e o montador. `t-tr-mestre-jogador.mjs` de 22 para 30 (o aviso, com o Talento Alma Inquebrável como fonte do Motor). `t-estados-organiza.mjs` passou a contar 64 estados e 39 com família (o Longo). `t-sistema.mjs` ganhou o `golpeEspecial` entre as consequências das Bases concedidas (457). Contraprova contra o commit anterior: os 14 casos que medem mudança falham lá e as três seções da sessão quebram (as funções não existem). Suíte: 115 arquivos, só o vermelho conhecido `t-invocacoes-motor.mjs` (1 falha, 84 ok, igual à linha de base). ESLint do Afty zerado, build limpo, nenhum travessão nas linhas novas. Canais: 100, 46 com alvo.

**Verificado ao vivo** (`/Player`, dev limpo, Combatente 20 com Espada Curta e Arco Longo): o montador soma Amplo, Letal e Penetrante em "Custo 6 PE" e "Pagar 3 PE"; a troca por 6 paga tudo (PE fica em 80) e fica desligada no resto da cena; Preciso e Sacrifício custam o mínimo de 1, abatido pelo Autossuficiente, a Vida cai de 126 para 111 e o Preciso passa a mostrar +2; o Longo leva a Espada Curta de 3m para 4,5m e o Arco Longo de 30/60 para 39/69, com o Básico parado em 1,5m, e aparece ligado na aba Buffs; os seis contadores aparecem na aba Habilidades e dois cliques deixam o Indomável em 8/10; com a Postura do Céu, a virada para a rodada 2 põe +2 de casca na barra de Preparo; no criador, a Vontade marcada à mão mostra "Sem Fonte". Console sem erro.
